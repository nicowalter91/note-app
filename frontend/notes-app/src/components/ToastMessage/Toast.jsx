...
const Toast = ({ isShown, message, type, onClose }) => {
  return (
    <div className={`absolute top-20 right-6 transition-all duration-400 w-full max-w-sm ${!isShown ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className='flex items-center gap-3 py-2 px-4'>
        ...
        <p className='test-sm text-slate-800'>Note Added Successfully</p>
      </div>
      </div>
    </div>
  )
